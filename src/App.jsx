       ...
       <div className="mx-3 md:container md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-1/2">
        <h1 className='font-bold text-center text-xl'>TodoTrack - Manage your todos at one place</h1>
         <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <button onClick={handleAdd} disabled={todo.length<=3} className='bg-slate-700 hover:bg-slate-800 disabled:bg-slate-400 p-2 py-1 text-sm font-bold text-white rounded-md'>Save</button>
         </div>
         ...
         <div className="todos">
          ...
          return (showFinished || !item.isCompleted) && <div key={item.id} className={"todo flex md:w-1/2 my-3 justify-between"}>
            ...
            <div className="buttons flex h-full">
              ...
              <button onClick={(e)=>{handleDelete(e, item.id)}} className='bg-slate-700 hover:bg-slate-800 p-2 py-1 text-sm font-bold text-white rounded-md mx-1'><AiFillDelete /></button>
            </div> 
          </div>
          })}
         </div>
